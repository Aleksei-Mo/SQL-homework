...
select MARK,COLOR, count(COLOR) as COLOR_Num from auto
...
where MARK="BMW"
union select count(MARK) from auto where MARK!="BMW"
group by MARK;

-- Даны 2 таблицы, созданные следующим образом.Напишите запрос, который вернет строки из таблицы test_a, id которых нет в таблице test_b, НЕ используя ключевого слова NOT.
